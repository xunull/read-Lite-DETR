...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
                 ...
                 # dim_feedforward在这里定义了两个 一个是enc 一个是dec dec在下面
        ...
        # 只能用deformable_encoder 下面的分支会抛错
        ...
        if deformable_encoder:
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward_enc,
                                                              ...
                                                              # 以下是Lite DETR多的参数
                                                              use_pytorch_version=use_pytorch_version,
                                                              value_proj_after=value_proj_after,
                                                              deformable_use_checkpoint=deformable_use_checkpoint,
                                                              same_loc=same_loc, proj_key=proj_key,
                                                              key_aware=key_aware)
            ...
            if small_expand:
                encoder_layer_last = DeformableTransformerEncoderLayer(d_model, dim_feedforward_enc,
                                                                       ...
                                                                       # small_expand 多了这两个参数
                                                                       enc_scale=enc_scale, small_expand=small_expand,
        ...
        if deformable_decoder:
            decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward_dec,
                                                              ...
                                                              # 多的参数
                                                              use_pytorch_version=use_pytorch_version,
                                                              value_proj_after=value_proj_after, key_aware=key_aware)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if use_deformable_box_attn:
            ...
            self.self_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points, use_pytorch_version=use_pytorch_version,
                                          value_proj_after=value_proj_after, proj_key=proj_key,
                                          deformable_use_checkpoint=deformable_use_checkpoint, same_loc=same_loc,
                                          key_aware=key_aware)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        if use_deformable_box_attn:
            ...
            self.cross_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points,
                                           use_pytorch_version=use_pytorch_version, value_proj_after=value_proj_after,
                                           proj_key=proj_key, key_aware=key_aware)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
